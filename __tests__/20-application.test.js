...
beforeEach(async () => {
  ...
  render(vdom);
  userEvent.click(await screen.findByText(/Chat/i));
  userEvent.click(await screen.findByText(/RU/i));
  userEvent.click(await screen.findByText(/RUSSIA/i));
});
...
describe('auth', () => {
  ...
  test('handle login error', async () => {
    server.use(rest.post('/api/login', (_req, res, ctx) => res(ctx.status(401))));
  ...
  test('successful login', async () => {
    server.use(
      ...
      rest.get('/api/data', mockInitialData),
    );
...
describe('registration', () => {
  test('handle new user creation', async () => {
    server.use(
      rest.post('/api/signup', mockSignup),
      rest.get('/api/data', mockInitialData),
    );
...
describe('chat', () => {
  ...
  test('rename channel', async () => {
    userEvent.click(await screen.findByRole('button', { name: '+' }));
    userEvent.type(await screen.getByLabelText(/Имя канала/i), 'test channel');
    userEvent.click(await screen.findByRole('button', { name: /Отправить/i }));
